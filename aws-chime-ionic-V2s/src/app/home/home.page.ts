...
export class HomePage implements OnInit, AudioVideoObserver, DeviceChangeObserver, ContentShareObserver, VideoDownlinkObserver {
  ...
  // ].join('');
  static readonly BASE_URL: string = 'https://harshmalve.com:8080/';
  ...
  initEventListeners(): void {
        ...
        async (): Promise<void> => {
          ...
          try {
            const response = await fetch(
              `${HomePage.BASE_URL}join?title=${encodeURIComponent(
                this.meeting
              )}&name=${encodeURIComponent(HomePage.DID)}&region=${encodeURIComponent(
                region
              )}`,
              {
                method: 'POST',
            ...
            this.sipURI = `sip:${HomePage.DID}@${this.voiceConnectorId};transport=tls;X-joinToken=${joinToken}`;
    ...
    const startLiveTranscription = async (engine: string, languageCode: string, region: string) => {
      const response = await fetch(`${HomePage.BASE_URL}start_transcription?title=${encodeURIComponent(this.meeting)}&engine=${encodeURIComponent(engine)}&language=${encodeURIComponent(languageCode)}&region=${encodeURIComponent(region)}`, {
        method: 'POST',
      });
      const json = await response.json();
      if (json.error) {
  ...
  async createLogStream(
    ...
    try {
      const response = await fetch(`${HomePage.BASE_URL}${pathname}`, {
        method: 'POST',
        body,
      });
          ...
          JSON.stringify({
            ...
            attributes: {
              ...
              meetingHistory: meetingHistory.filter(({ timestampMs }) => {
                return Date.now() - timestampMs < HomePage.MAX_MEETING_HISTORY_MS;
              }),
            },
          })
        );
  ...
  async initializeMeetingSession(configuration: MeetingSessionConfiguration): Promise<void> {
    ...
    if (this.isLocalHost()) {
      ...
      this.meetingSessionPOSTLogger = new MeetingSessionPOSTLogger(
        'SDK',
        configuration,
        HomePage.LOGGER_BATCH_SIZE,
        HomePage.LOGGER_INTERVAL_MS,
        `${HomePage.BASE_URL}logs`,
        this.logLevel
      );
      ...
      this.meetingEventPOSTLogger = new MeetingSessionPOSTLogger(
        ...
        HomePage.LOGGER_BATCH_SIZE,
        HomePage.LOGGER_INTERVAL_MS,
        `${HomePage.BASE_URL}log_meeting_event`,
        this.logLevel
      );
    }
  ...
  async setupEventReporter(configuration: MeetingSessionConfiguration): Promise<EventReporter> {
    ...
    if (this.isLocalHost()) {
      ...
      const eventReportingPOSTLogger = new MeetingSessionPOSTLogger(
        ...
        HomePage.LOGGER_BATCH_SIZE,
        HomePage.LOGGER_INTERVAL_MS,
        `${HomePage.BASE_URL}log_event_ingestion`,
        LogLevel.DEBUG
      );
  ...
  async endMeeting(): Promise<any> {
    await fetch(`${HomePage.BASE_URL}end?title=${encodeURIComponent(this.meeting)}`, {
      method: 'POST',
    });
  }
  ...
  async getAttendee(attendeeId: string): Promise<any> {
    const response = await fetch(
      `${HomePage.BASE_URL}attendee?title=${encodeURIComponent(
        this.meeting
  ...
  private async toggleLiveTranscription(): Promise<void> {
    ...
    if (this.enableLiveTranscription) {
      const response = await fetch(`${HomePage.BASE_URL}${encodeURIComponent('stop_transcription')}?title=${encodeURIComponent(this.meeting)}`, {
        method: 'POST',
      });
      const json = await response.json();
      if (json.error) {
  ...
  private initContentShareDropDownItems(): void {
    ...
    item.addEventListener('click', () => {
      ...
      this.contentShareStart(HomePage.testVideo);
    });
